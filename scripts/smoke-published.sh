...
for command in start plan adopt channel next reply surface attest stop shutdown runs status host monitor watch unwatched stop-guard ask results goals transcript agents telemetry; do
  case "$help" in
    *"$command"*) ;;
