...
# runs this after every release, on every OS, for both registries, and anything it
# had to install first would be a second thing that can rot.
#
...
for command in start plan adopt channel next reply surface attest stop shutdown runs status host monitor watch unwatched stop-guard ask publish-branch repo-recover results goals transcript agents telemetry; do
  case "$help" in
    *"$command"*) ;;
