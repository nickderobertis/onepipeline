...
//
// llmlint: ignore[code_lands_in_the_domain_that_owns_it] the *producer* domains own their
// own words and keep them — `onevcs::SOURCE_WORD` is `onevcs`'s and
// `oneagentgraph::event::SOURCE_WORD` is the graph's, each declared beside the vocabulary
// that stamps it, and neither is restated here. What is here is the **merged stream's**
// own vocabulary, and that is this crate's domain: `onepipeline` is the composition layer
// and the only crate that links every producer, so a closed set naming all three is not
// nameable anywhere else, and putting it in a producer would invert the dependency
...
// llmlint: ignore[code_lands_in_the_domain_that_owns_it] a composition rather than a
// registry bag: each document is generated by the domain that owns it — the event
// families by this module, `agent.note@1` by `onejudge::note::Note`, the transport
// protocol by `onemessagebus::transport` — and this function only *lists* them under the
// ids a record has always validated against. Somewhere has to hold that list, and it is
// this crate for the reason the `Source` directive above states. The alternative, three
// registries a reader composes at each call site, is what the bus's retired agent-profile
// crate was and is what this change removes; `tests/registry.rs` holds the composed set to
// a capture of exactly what that crate produced.
#[must_use]
...
// llmlint: ignore[invalid_states_unrepresentable] the bus's own `BundleVersion` has no
// const constructor — it is reached through `FromStr` — so a module constant cannot hold
...
// state for long: `events_bundle` parses it on first use, `the_committed_events_bundle_is_the_compiled_in_vocabulary`
// reads the committed file back through the bus's own `SchemaBundle` and asserts its
// version equals this, and the contract paragraph naming it is held to the same string —
// so a value that is not a bundle version fails the deterministic tier rather than
// reaching a link.
