...
// llmlint: ignore-block[code_lands_in_the_domain_that_owns_it] the *producer* domains own
// their own words and keep them: `onevcs::SOURCE_WORD` and `onevcs::Phase` are `onevcs`'s,
// `oneagentgraph::event::SOURCE_WORD` is the graph's, each declared beside the vocabulary
// that stamps it, and neither is restated here — `Phase` is re-exported from its owner
// above. What this block holds is the **merged stream's** own vocabulary, and that is this
// crate's domain: `onepipeline` is the composition layer and the only crate that links
// every producer, so a closed set naming all three, and a table of what each writes, is not
// nameable anywhere else. Putting either in a producer would invert the dependency
// direction `AGENTS.md` fixes. `tests/contract.rs`'s drift check holds each word here to
// the producer that owns it, so a fourth producer is a compile-and-check step here rather
...
// Closed, unlike each producer's own open source: a word outside it is a stream this crate
// has no reader for, which is why a sibling envelope carrying one is refused at the
// crossing and reported rather than dropped.
//
...
// llmlint: ignore-block[code_lands_in_the_domain_that_owns_it] a composition rather than a
// registry bag: every document here is generated by the domain that owns it, and none is
// restated — `agent.note@1` is `onejudge::note::Note`'s and the transport protocol is
// `onemessagebus::transport`'s. What this function owns is the *list*, which is the merged
// stream's own fact for the reason the directive over `Source` above states. The
// alternative — three registries each reader composes for itself — is what the bus's
// retired agent-profile crate was, and `tests/registry.rs` holds this composed set to a
// capture of exactly what that crate produced.
#[must_use]
...
// llmlint: ignore-block[invalid_states_unrepresentable] the bus's own `BundleVersion` has
// no const constructor — it is reached through `FromStr` — so a module constant cannot hold
// one, and this is the same shape `channel::layout::DOCUMENT_VERSION` beside it already
// publishes for `schemas/planner-channel.json`. The text is never a representable invalid
// state for long: `events_bundle` below parses it on first use, and the contract test
// `the_committed_events_bundle_is_the_compiled_in_vocabulary` reads the committed file back
