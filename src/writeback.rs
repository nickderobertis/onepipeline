...
// Closeout deliberately does not inherit COMMAND_LIMIT. A slow store may keep retrying in
// the worker, but it still cannot turn a completed graph into a ten-second run settlement.
