...
#[test]
fn live_holders_refuse_unless_acknowledged_and_stale_ones_are_reported_or_forgotten() {
    ...
    // supported way to ask. There the record is answered on its owner alone and
    // is forgotten instead, which is the case below.
    #[cfg(unix)]
    {
    ...
    // has already exited, on a run root nothing is working inside. Since `onevcs`
    // 0.16.3 the sibling forgets such a record as it enumerates, so it never
    // reaches this launcher — and the launch says nothing about it rather than
    // printing a holder an operator cannot act on. Seven of those above a launch
    // is what made one real refusal read like seven ignorable ones.
...
/// is working inside — which is what `onevcs` calls **spent** and forgets.
///
