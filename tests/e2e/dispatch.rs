...
/// The double refuses a hold it cannot wait out, instead of dying inside one.
///
/// The rendezvous bound arrives in the environment, and every journey that holds
/// a dispatch open sets it — so a mistyped one is a live way for this suite to be
/// misconfigured. Read without a ceiling it becomes a duration added to a clock,
/// and an addition past what an `Instant` can represent panics: the double
/// unwinds mid-turn, and what reaches the journey is a harness that died saying
/// nothing about the value it was handed. That is the one failure this binary
/// cannot report, because reporting is all it does. So the bound is checked where
/// it is read, and this is the surface that says so.
#[test]
fn the_model_turn_double_refuses_a_hold_it_cannot_wait_out() {
    let world = World::new("rendezvous-bound");
    // Scripted to hold: the bound is read only by a turn that waits, so a turn
    // that runs straight through would pass whatever the value was.
    std::fs::write(world.fakes.join("turn.hold"), "hold").expect("the hold is scripted");
    let held = |seconds: &str| {
        std::process::Command::new(crate::harness::double("fake-claude"))
            .args([
                "-p",
                "Do build.",
            ...
            .env(onepipeline_testfakes::SCRIPT_DIR_ENV, &world.fakes)
            .env("ONEPIPELINE_FAKE_RENDEZVOUS_SECONDS", seconds)
            .output()
            .expect("the double runs")
    };

    let refused = held(&u64::MAX.to_string());
    let said = String::from_utf8_lossy(&refused.stderr).to_string();
    assert!(
        !said.contains("panicked"),
        "the double panicked on the bound instead of reporting it: {said}"
    );
    assert_eq!(
        refused.status.code(),
        Some(i32::from(onepipeline_testfakes::MISCONFIGURED)),
        "a hold no clock can be advanced by was not refused as the misconfiguration \
         it is: {said}"
    );
    assert!(
        said.contains(&u64::MAX.to_string()),
        "the refusal does not name the value it was handed, so nothing tells the \
         journey which variable to fix: {said}"
    );

    // Released before it is run, so the same double with a bound it can hold
    // completes the turn: the refusal above is about the value rather than about
    // holding at all.
    for released in ["turn.go", "turn.settle"] {
        std::fs::write(world.fakes.join(released), "go").expect("the rendezvous is released");
    }
    let ran = held("30");
    assert_eq!(
        ran.status.code(),
        Some(0),
        "a bound this double can wait out was refused too: {}",
        String::from_utf8_lossy(&ran.stderr)
    );
}
