...
impl World {
    ...
    pub fn cmd(&self, args: &[&str]) -> Command {
        ...
        command
            ...
            // A `change-auto` publication watches the change request through to
            // the merge the host performs, polling and giving up on its own
            // clock. Behind the stand-in there is nothing to wait for — what the
            // host will do is decided before the run starts — so the sibling's
            // half-minute poll and hour-long bound would be a suite that sleeps
            // where it means to assert. Both are the sibling's own knobs, set
            // small rather than worked around.
