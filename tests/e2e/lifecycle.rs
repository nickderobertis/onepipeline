...
///
/// A refused publishing push is a **preserving** failure — the tree that was
/// rejected is still on the branch — so it settles `push-rejected` rather than
/// the residual, and the budget is set to one so the journey stays about the two
/// endings rather than about the loop that answers one of them.
#[test]
fn a_publication_its_merge_path_refuses_settles_the_node_failed_by_name() {
    let world = World::new("lifecycle-refused").with_env("ONEPIPELINE_PUBLICATION_ATTEMPTS", "1");
    ...
    assert_eq!(result["nodes"][0]["outcome"], "push-rejected");
    let reported = world.run(&["results", &run]);
    reported.exited(0).out_has("push-rejected");
    ...
    // And the budget is the bound: one attempt was asked for and one was made, so
    // the settlement above is a single publication's two endings rather than the
    // last of several folded together.
    assert_eq!(
...
/// A publication `onevcs` refuses outright settles the residual and is **not**
/// asked again.
///
/// The other side of the line from every journey above: those are failures a
/// further attempt on the same branch could answer, and this is one no attempt
/// can. The repository is hosted somewhere this build has no `RemoteHost` for
/// while its rules ask for a change request, which is the seam `onevcs` reports
/// `NotImplemented` at — it will say the same thing however many times it is
/// asked, so the node settles under [`Failure::RESIDUAL`]'s word and the loop
/// does not spend a second workstream reproducing an answer already in hand.
///
/// Written against a misconfiguration rather than against a gate: this used to be
/// the journey a repository's own rejecting gate drove, and `onevcs` 0.11.0 runs
/// no gate — a merge path that refuses is `push-rejected` and preserving. What
/// still reaches the terminal side is a request the sibling will not take at all.
#[test]
fn a_publication_onevcs_refuses_outright_settles_the_residual_and_is_not_retried() {
    let world = World::new("lifecycle-residual");
    let repo = world.repository("change-open", &[]);
    // Re-registered against a host that is not github.com, so the identity is
    // hosted and a `change-*` policy has somewhere to open a change request in
    // principle — and no implementation to open it with. The operator's own
    // mistake, made the way one is made: the rules and the registration disagree,
    // and neither is wrong on its own.
    world.register(
        &repo.checkout,
        Some("https://git.example.invalid/owner/service.git"),
    );
    world.script("service.work", "the worker wrote this\n");
    let run = settle(&world, "residual", vec![lifecycle("service", &[])]);
    ...
    // Nothing was published, so no landing is claimed either way.
    assert_eq!(node["landing"], json!(null), "{node}");
    world
        .run(&["results", &run])
        .exited(0)
        .out_has("publication-failed");
    // The sibling's own words, which is what tells an operator which half of
    // their configuration to change.
    let detail = world.events_of(&run, "node-settled")[0]["payload"]["detail"]
        .as_str()
        .expect("the settlement says why")
        .to_string();
    assert!(
        detail.contains("onevcs:") && detail.contains("not implemented"),
        "the settlement does not say what onevcs refused: {detail}"
    );

    // And it was asked exactly once. A second dispatch here would spend a whole
    // workstream reproducing a refusal that is already in hand.
    assert_eq!(
        dispatches_of(&world, &run, "service").len(),
        1,
...
/// How many `gh` invocations a host outage has to refuse for the
/// **publication's** first read of the host to be the one refused.
///
/// The closeout asks the host whether the session already holds a change
/// request before anything is drafted or published — that is what decides
/// whether it finishes a change request the worker opened or opens one — and
/// that read is the first invocation a lifecycle node makes. So an outage that
/// ends after one call ends on the closeout's read, and the publication after it
/// meets a host that answers; two is the closeout's read and the publication's
/// own, which is the ending every journey scripting this is about.
const PUBLICATION_READ_OUTAGE: &str = "2";
...
///
/// The distinction the word exists for, and the recovery it asks for. The refusal
...
fn a_merge_path_that_goes_dark_and_comes_back_is_answered_by_reading_it_again() {
    let world = World::new("lifecycle-rereadhost")
        // Long enough that a re-dispatch could not be mistaken for a re-read, and
        // no wait between the reads: what is being proved is the routing, and a
        // journey that slept for it would only be slower.
        .with_env("ONEPIPELINE_PUBLICATION_ATTEMPTS", "2")
        .with_env("ONEPIPELINE_MERGE_PATH_BACKOFF_SECONDS", "0");
    let repo = world.repository("change-auto", &[]);
    world.script("service.work", "the worker wrote this\n");
    world.script("gh.merged", "");
    world.script("gh.outage", PUBLICATION_READ_OUTAGE);
    ...
    // And it cost one dispatch. This is the whole of the change: the agent was
    // never asked for a fresh clone and a fresh complete gate to re-push what the
    ...
    // What *was* asked again is the host, which is what makes the recovery a read.
    let asked = world
        .invocations()
        .into_iter()
...
/// back — and the node has to settle, because a run that polled somebody else's
/// API forever would be worse than one that reported honestly. What it says is
/// the three things a person needs and none they have to infer: the branch, the
/// commit it landed at, and what stopped the read. It is still not re-dispatched:
/// the tree was never rejected, so there is nothing for a worker to change.
#[test]
fn a_merge_path_that_never_answers_settles_the_node_saying_where_the_work_is() {
    let world = World::new("lifecycle-pushedunverified")
        // Two reads, so the settlement has a number to report that is not one, and
        // no wait between them.
        .with_env("ONEPIPELINE_MERGE_PATH_READS", "2")
        .with_env("ONEPIPELINE_MERGE_PATH_BACKOFF_SECONDS", "0")
        .with_env("ONEPIPELINE_PUBLICATION_ATTEMPTS", "2");
    let repo = world.repository("change-auto", &[]);
    world.script("service.work", "the worker wrote this\n");
    ...
    // And it is in the checkout too, which is where a person picks it up.
    assert!(
        repo.has_branch(&world, branch),
        "the branch an unread merge path was left on was not handed back"
    );

    // The commit rides the settlement, so a person is told what to read rather
    // than left to go and find the branch's tip themselves. It is the tip the
    // origin has, because that is the push this failure is about.
    let head = node["head"]
        .as_str()
        .expect("the node names the commit its branch landed at");
    assert_eq!(
        head,
        git(&world, &repo.origin, &["rev-parse", branch]).trim(),
        "the commit the settlement names is not the one on the origin"
    );

    // Not re-dispatched. The tree was never rejected, so asking a worker for it
    // again would buy a fresh clone and a fresh gate to re-push what is there.
    let dispatched = dispatches_of(&world, &run, "service");
    assert_eq!(
        dispatched.len(),
        1,
        "the agent was dispatched again for a tree nothing rejected\n{}",
        why(&world, &run)
    );

    // And an operator reads all of it without opening the store: the word, the
    // sibling's own sentence about where the work landed and what stopped the
    // read, and how many reads were spent on it.
    ...
    for said in [
        ...
        "is on origin",
        "api.github.com",
        "the merge path was read 2 times and never answered",
    ] {
        results.out_has(said);
    }
}
...
///
/// The window is held open rather than raced for: the repository's own `pre-push`
/// hook blocks until this test releases it, so the cancel lands while the
/// publication is still running and everything after the release — the push, its
/// refusal, the preserving classification — happens unconditionally. Dispatched
...
fn a_cancel_that_lands_before_the_next_attempt_settles_on_the_publication_failure() {
    ...
    // holds: `merge-queued` is a `change-*` record the host writes *after* the
...
///
/// The node override rather than a second world, so the two halves stay side by
/// side in one `runs` listing, which is the last assertion here.
#[test]
...
///
/// Four nodes in one session settled `task-failed` twenty seconds after their
/// final report, with thirteen commits, a green gate, and a clean tree on the
/// branch each had left behind — because a rate limit hit the harness, not
/// because any agent failed anything. Read as `task-failed`, every one of them
/// says "re-run this", and the evidence that the work was finished had to be dug
/// out of `events.jsonl` by hand.
///
/// So the word is chosen by classifying the dispatch's **own detail** — what
/// `oneagentgraph` writes on stderr when a member dies — and the branch and the
/// commit ride the settlement as fields rather than deciding it. This journey is
/// the half with work to point at; the one with none is in `boundary.rs`, and the
/// two reach the same word.
///
/// The worker publishes its own branch first, under a `local-direct` identity, so
/// the commit `onevcs` recorded for that branch is a fact this journey reads out
/// of the sibling's own stream rather than one it arranges — and no change
...
fn a_dispatch_that_died_rather_than_failing_its_task_settles_naming_what_killed_it() {
    ...
    world.script(
        "service.publishes",
        "chore: what the dispatch had finished when it died",
    );
    // The sentence `oneagentgraph` exits on when the harness under a member dies
    // — the operation it was in, and oneharness's own classification of why.
    world.script(
        "service.died",
        "provider error (respond): harness failed (rate_limit)",
    );
    let run = settle(&world, "dispatchdied", vec![lifecycle("service", &[])]);
    ...
    assert_eq!(settled["payload"]["status"], "failed", "{settled}");
    assert_eq!(
        settled["payload"]["outcome"], "dispatch-died",
        "a dispatch its harness killed was reported as an agent that failed its task: {settled}"
    );

    // Every view a manager decides from carries the classification and the work.
    let node = world.run_json(&run, "result.json")["nodes"][0].clone();
    assert_eq!(node["status"], "failed", "{node}");
    assert_eq!(node["outcome"], "dispatch-died", "{node}");
    // The producer's own word, lifted out of its own sentence and not
    // re-vocabularised here.
    assert_eq!(node["cause"], "rate_limit", "{node}");
    // Nothing was reported landed and no change request was offered: this is a
    // failure, and the word says only that it was not the agent's.
    assert!(node["change_url"].is_null(), "{node}");

    // The branch, and what is on it. The commit is the sibling's own record of
    // where it left that branch, which is the fact a manager had to read a
    // journal to get.
    let branch = node["branch"].as_str().expect("the node names its branch");
    let head = node["head"]
        .as_str()
...
fn gh_calls(world: &World) -> Vec<Vec<String>> {
    world
        ...
        .map(|call| serde_json::from_value(call["args"].clone()).expect("the argv"))
        .collect()
}
...
/// The `turn-activity` records one node's dispatches under one persona wrote,
/// which is where a double reports the task it was handed and the environment
/// it read.
fn turns_of(world: &World, run: &str, node: &str, persona: &str) -> Vec<serde_json::Value> {
    world
        .journal(run)
        .into_iter()
        .filter(|event| {
...
fn drafting_task_of(world: &World, run: &str, node: &str) -> String {
    ...
    assert_eq!(
        turns.len(),
        1,
        "the drafting dispatch did not run exactly once: {turns:?}"
    );
    turns[0]["payload"]["task"]
        .as_str()
        .expect("the drafting dispatch echoes its task")
        .to_string()
}

/// The opening sentence every drafting dispatch is given, byte for byte: the
/// consumer host reads it out of this binary to hold its own drafter to the same
/// prompt, so a journey holds the composed task to it rather than to a prefix.
...
/// The journey the whole change exists for: a worker opened its session's change
/// request as a **draft** while it worked and started the description, and the
/// closeout finishes that change request rather than opening another — the
/// drafter is shown it, the drafted description is written onto it under the
/// plan's title, and the same publication that lands the change lifts the draft.
///
/// Every half is held against the host, through the real `onevcs`: one change
/// request and never a second, the description it carries after the closeout,
/// the lift, and the node settling on the very word a fresh publication under
/// `change-open` settles on — told apart from one only by its settlement detail.
#[test]
fn the_closeout_finishes_a_change_request_the_worker_opened_as_a_draft() {
    let world = World::new("lifecycle-worker-draft");
    world.repository("change-open", &[]);
    world.script("service.work", "the worker wrote this\n");
    // The worker's own draft: its own title, and a description it started.
    ...
    let drafting = world.pr_author_graph();
    let node = titled(lifecycle("service", &[]), "feat: land what the worker made");
    let path = world.plan("finished", &plan_of("finished", vec![node]));
    world
        .run(&["start", &path, "--attach", "--pr-author-graph", &drafting])
        .settled();
    let run = "finished";

    // One change request on the host, opened by the worker as a draft, and no
    // second one beside it for the same branch.
    ...
    // The host now carries the drafted description under the plan's own title,
    // whatever the worker opened it under.
    assert_eq!(
        opened[0]["body"],
        "## What\nFinished from the diff and the worker's transcript.",
        "the drafted description did not reach the change request: {opened:?}\n{}",
        why(&world, run)
    );
    assert_eq!(opened[0]["title"], "feat: land what the worker made");
    assert_eq!(
    ...
    // The node settles on the publication's own words — the same word a fresh
    // `change-open` publication settles on — and its detail is what tells it
    // apart from one.
    let node = world.run_json(run, "result.json")["nodes"][0].clone();
    assert_eq!(node["status"], "done", "{node}\n{}", why(&world, run));
    assert_eq!(node["outcome"], "change-open", "{node}");
    ...
    let url = node["change_url"]
        .as_str()
        .unwrap_or_else(|| panic!("the node names no change request: {node}"))
        .to_string();
    assert!(
        url.ends_with(&format!("/pull/{}", opened[0]["number"])),
        "the node names a change request the host did not open: {url} vs {opened:?}"
    );
    let url = url.as_str();
    let settled = world.events_of(run, "node-settled");
    ...
    // The drafting dispatch was handed the composed task: the opening sentence
    // byte for byte, the node's task, the change request as the worker left it,
    // and how to read the worker's transcript — in that order.
    let task = drafting_task_of(&world, run, "service");
    assert!(task.starts_with(DRAFTING_TASK), "{task}");
    let change_section = format!(
        "\n\n## Change request\n{url}\nHeld as a draft by the worker: yes\n\n\
         ### Description as the worker left it\n## What\nHalf written by the worker.\n"
    );
    let at = task
        .find(&change_section)
        .unwrap_or_else(|| panic!("the drafting task does not show the change request:\n{task}"));
    let transcript = format!("\n\n## Worker transcript\n`onepipeline transcript {run} service`");
    let after = task.find(&transcript).unwrap_or_else(|| {
        panic!("the drafting task does not say how to read the transcript:\n{task}")
    });
    assert!(at < after, "the sections are out of order:\n{task}");
    assert!(
        task.contains("ONEPIPELINE_RUN_ID") && task.contains("ONEPIPELINE_RUNS_DIR"),
        "the drafting task does not name what the transcript verb reads:\n{task}"
    );
    let task_section = task.find("## What").expect("the node's task is rendered");
    assert!(
        task_section < at,
        "the node's task does not precede the change request:\n{task}"
    );
    ...
    assert_eq!(
        ...
        "feat: land what the worker made"
    );
    assert_eq!(described["payload"]["url"], json!(url));

    // And `results` and `status` show the node as one that published, with the
    // detail beside it.
    world
        .run(&["results", run])
        .exited(0)
