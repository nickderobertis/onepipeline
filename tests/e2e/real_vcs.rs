...
fn a_merge_path_that_refuses_the_publishing_push_fails_the_node_and_leaves_the_base_alone() {
    ...
    // `push-rejected` and not the residual: the tree the merge path turned down
    // is still on the branch the session handed back, which is a failure a
    // further attempt can answer.
    assert_eq!(
        result["nodes"][0]["outcome"],
        "push-rejected",
        "{result}\n{}",
