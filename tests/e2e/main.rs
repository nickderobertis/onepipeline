...
// site the rule reads: the module is measured at about 20 seconds against a binary that
// already holds three minute-long journeys, and what it exercises is `views`, `vcs` and
// `rendercost` — so a project edged narrower than the crate would drop it out of `nx
